...
tests =
  testGroup
    ...
    [ testCaseWithTimeout "ping-pong" $ withQUICTransport $ flip Tests.testPingPong 5,
      ...
      -- This test is very flaky specifically in Github Actions
      veryFlaky $ testCaseWithTimeout "parallelConnects" $ withQUICTransport $ flip Tests.testParallelConnects 5,
      ...
      testGroup
        "QUIC-specific tests"
        [ testSendVeryLargeMessages mkQUICTransport
        ]
    ]
...
veryFlaky :: TestTree -> TestTree
veryFlaky = flakyTest (limitRetries 3)
...
testSendVeryLargeMessages :: IO (Either String Transport) -> TestTree
testSendVeryLargeMessages getTransport = testCase "Send very large messages" $ do
  transport <- either assertFailure pure =<< getTransport
  server <- spawn transport echoServer
  result <- newEmptyMVar

  let numPings = 10
  let bigMessage = BS.replicate 4096 66
  _ <- forkTry $ do
