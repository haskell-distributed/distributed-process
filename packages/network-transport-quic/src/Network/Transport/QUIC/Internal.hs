                            ...
                            Just ourEndPoint -> do
                                readMVar (ourEndPoint ^. localEndPointState) >>= \case
                                    ...
                                    LocalEndPointStateValid _ -> do
                                        ...
                                        -- TODO: this may not be the first connection from this remote address.
                                        -- In this case, we want to re-use the existing remote endpoint
...
-}
handleIncomingMessages :: (HasCallStack) => LocalEndPoint -> RemoteEndPoint -> IO ()
handleIncomingMessages ourEndPoint remoteEndPoint =
  ...
  where
    ...
    connectionId connId = createConnectionId serverConnId connId
                ...
                Right CloseEndPoint -> do
                    connIds <- modifyMVar (remoteEndPoint ^. remoteEndPointState) $ \case
                        RemoteEndPointValid vst -> do
                            -- TODO: check how many connections are outgoing
...
connectToSelf ourEndPoint = do
  ...
  where
    ...
    selfSend connAlive connId msg =
        try . withMVar (ourEndPoint ^. localEndPointState) $ \st -> case st of
            LocalEndPointStateValid _ -> do
    ...
    selfClose connAlive connId =
        withMVar (ourEndPoint ^. localEndPointState) $ \st -> case st of
            LocalEndPointStateValid _ -> do
                alive <- readIORef connAlive
                when alive $ do
