...
data TransportState = State { _channels    :: Map Address (Chan Event) 
                            , _connections :: Map Address Int
                            , _multigroups :: Map MulticastAddress (MVar [Chan Event])
                            }

channels :: Lens TransportState (Map Address (Chan Event))
channels = lens _channels (\ch st -> st { _channels = ch })

connections :: Lens TransportState (Map Address Int)
...
channelAt :: Address -> Lens TransportState (Chan Event) 
channelAt addr = channels >>> at addr

connectionAt :: Address -> Lens TransportState Int
...
chanNewEndPoint state = do
  ...
  addr <- modifyMVar state $ \st -> do
    let addr = Address . BSC.pack . show . Map.size $ st ^. channels
...
chanNewMulticastGroup :: MVar TransportState -> Chan Event -> IO (Either Error MulticastGroup)
...
chanResolveMulticastGroup :: MVar TransportState -> Chan Event -> MulticastAddress -> IO (Either Error MulticastGroup)
chanResolveMulticastGroup state endpoint addr = do
  group <- (^. (multigroups >>> mapLens addr)) <$> readMVar state 
  case group of
    Nothing   -> return . Left  $ Error undefined ("Group " ++ show addr ++ " not found")
