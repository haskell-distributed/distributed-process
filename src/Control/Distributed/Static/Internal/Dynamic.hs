module Control.Distributed.Static.Internal.Dynamic
  ( -- * The @Dynamic@ type
    Dynamic
...
unsafeToDyn :: a -> Dynamic
unsafeToDyn x = Dynamic (error "No type rep") (unsafeCoerce x)

unsafeCastDyn :: (TypeRep -> TypeRep) -> Dynamic -> Dynamic
unsafeCastDyn f (Dynamic t x) = Dynamic (f t) x
